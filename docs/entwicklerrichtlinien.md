...
|---|---|---|
| Variablen, Funktionen | snake_case | `calculate_total_power` |
| Klassen, Typen | PascalCase | `DeviceController` |
| Konstanten | UPPER_SNAKE_CASE | `MAX_RETRY_COUNT` |
| Dateien / Module | snake_case | `device_controller.py` |
...
├── custom_components/battery_bridge/     # Produktivcode — die HA-Integration
├── tests/                                 # Tests, Struktur spiegelt den Quellcode
└── docs/                                  # diese Doku
```

Details zur inneren Struktur von `custom_components/battery_bridge/`:
...
- Versionen werden gepinnt (`uv.lock`).
- Eine neue Laufzeit-Abhängigkeit ist eine Design-Entscheidung → Eintrag in
  [design-entscheidungen.md](design-entscheidungen.md).

## Formatierung

Formatierung erledigt das Tooling, nicht die Diskussion: `ruff check .`. Manuelles Abweichen vom
